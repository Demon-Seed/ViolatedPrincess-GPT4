...
(function () {
  ...
  SceneManager.snapForThumbnail = function () {
    if (this._scene) {
      ...
      this._thumbnailBitmap = Bitmap.snap2(
        this._scene,
        thumbSaveWidth,
        thumbSaveHeight,
        cw,
   ...
   */
  Window_Base.prototype.drawBase64Data = function (
    src,
    cacheKey,
    x,
    y,
   ...
   * @param {Function} onDrawAfter 描画後の処理
   */
  Window_Base.prototype._onLoadBase64Data = function (
    bitmap,
    x,
    y,
    lastOpacity,
   ...
   * @param {Function} onDrawAfter 描画後の処理
   */
  Window_Selectable.prototype.drawBase64Data = function (
    src,
    cacheKey,
    x,
    y,
    ...
    bitmap.addLoadListener(() => {
      if (
        this.topRow() !== lastTopRow ||
        Math.floor(this._scrollX / this.itemWidth()) !== lastLeftCol
      ) {
        return;
      }
      this._onLoadBase64Data(
  ...
  if (isShowInList) {
    Window_SavefileList.prototype.thumbnailX = eval(
      "(function(rect, width) { return %1; });".format(thumbItemPosX)
    );
    Window_SavefileList.prototype.thumbnailY = eval(
      "(function(rect, height) { return %1; });".format(thumbItemPosY)
    );
     ...
     * @param {boolean} valid
     */
    Window_SavefileList.prototype.drawThumbnail = function (
      info,
      thumbRect,
      valid
    ) {
      ...
      if (savefileId > 0 && info.thumbnail) {
        ...
        if (!thunmbBitmap.isReady()) {
          ...
          const empty = ImageManager.loadBusyThumbBitmap(
            thumbRect.width,
            thumbRect.height
          );
          sprite.bitmap = empty;
        ...
        thunmbBitmap.addLoadListener(() => {
          ...
          if (this.topIndex() <= listIndex && this.bottomIndex() >= listIndex) {
            ...
            sprite.bitmap.paintOpacity = valid
              ? 255
              : this.translucentOpacity();
          } else {
  ...
  if (otherWindowClass) {
    ...
    if (
      !AnyWindowClass ||
      !AnyWindowClass.prototype ||
      !(AnyWindowClass.prototype instanceof Window_Base)
    ) {
      throw new Error(
        AnyWindowClass + ": This is not a class extended 'Window_Base' class"
      );
    }

    AnyWindowClass.prototype._thumbnailX = eval(
      "(function(rect, width) { return %1; });".format(thumbOtherPosX)
    );
    AnyWindowClass.prototype._thumbnailY = eval(
      "(function(rect, height) { return %1; });".format(thumbOtherPosY)
    );
      ...
      if (this._savefileId > 0 && info && info.thumbnail) {
        ...
        thunmbBitmap.addLoadListener(() => {
          ...
          if (savefileId === this._savefileId) {
            ...
            this._thumbSprite.bitmap.paintOpacity = valid
              ? 255
              : this.translucentOpacity();
          }
        });
