...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (pluginCommandMethod) {
      this[pluginCommandMethod](
        this.convertAllSelfVariables(convertAllArguments(args))
      );
    }
  };
  ...
  Game_Interpreter.prototype.callMapEventById = function (pageIndex, eventId) {
    ...
    if (event) {
      this.setupAnotherList(
        param.KeepEventId ? null : eventId,
        event.getPages(),
        pageIndex
      );
    }
  ...
  Game_Interpreter.prototype.callMapEventByName = function (
    ...
    if (event) {
      this.setupAnotherList(
        param.KeepEventId ? null : event.id,
        event.pages,
        pageIndex
      );
    }
  ...
  Game_Interpreter.prototype.controlSelfVariable = function (
    index,
    type,
    operand,
    formulaFlg
  ...
  Game_Interpreter.prototype.controlSelfVariableRange = function (
    startIndex,
    endIndex,
    type,
    operand,
    ...
    var character = this.character(0);
    if (character) {
      character.controlSelfVariableRange(
        startIndex,
        endIndex,
        type,
        operand,
  ...
  Game_Interpreter.prototype.convertAllSelfVariables = function (args) {
    return args.map(function (arg) {
      return $gameSelfSwitches.convertSelfVariableCharacter(
        this._eventId,
        arg,
        false
      );
  ...
  Game_SelfSwitches.prototype.convertSelfVariableCharacter = function (
    eventId,
    text,
    scriptFlag
  ) {
  ...
  Game_Event.prototype.setTemplate = function (event) {
    ...
    if (templateId) {
      ...
      this._override =
        param.AutoOverride || !!getMetaValues(event, ["OverRide", "上書き"]);
  ...
  Game_Event.prototype.integrateNote = function (event, type) {
    ...
    this._templateEvent.note =
      (type === 1 ? this._templateEvent.note : "") + event.note;
  ...
  Game_Event.prototype.getOriginalPages = function () {
    var eventId = isExistPlugin("SAN_MapGenerator")
      ? this._dataEventId
      : this._eventId;
  ...
  Game_Event.prototype.meetsConditions = function (page) {
    return (
      _Game_Event_meetsConditions.apply(this, arguments) &&
      this.meetsConditionsForSelfVariable(page)
    );
  };
  ...
  Game_Event.prototype.meetsConditionsForSelfVariable = function (page) {
    ...
    return !(
      comment && this.execConditionScriptForSelfVariable(comment) === false
    );
  };
  ...
  Game_Event.prototype.execConditionScriptForSelfVariable = function (note) {
    ...
    return scripts.every(function (script) {
      ...
      script = $gameSelfSwitches.convertSelfVariableCharacter(
        this._eventId,
        script,
        true
      );
  ...
  Game_Event.prototype.controlSelfVariable = function (
    index,
    type,
    operand,
    formulaFlg
  ...
  Game_Event.prototype.controlSelfVariableRange = function (
    startIndex,
    endIndex,
    type,
    operand,
  ...
  Game_Event.prototype.operateSelfVariable = function (
    key,
    operationType,
    value
  ) {
