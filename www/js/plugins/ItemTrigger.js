...
(function () {
  "use strict";
  var pd_IT_useVariableId = Number(
    PluginManager.parameters("ItemTrigger")["ItemId_Variable"]
  );
  var pd_IT_Scene_ItemBase_checkCommonEvent =
    Scene_ItemBase.prototype.checkCommonEvent;
  Scene_ItemBase.prototype.checkCommonEvent = function () {
    if (pd_IT_useVariableId != 0) {
      if (
        !this.pd_IT_checkUseItemEvent($gamePlayer._x, $gamePlayer._y, false)
      ) {
        var x = $gameMap.roundXWithDirection(
          $gamePlayer._x,
          $gamePlayer._direction
        );
        var y = $gameMap.roundYWithDirection(
          $gamePlayer._y,
          $gamePlayer._direction
        );
        if (
          !this.pd_IT_checkUseItemEvent(x, y, true) &&
      ...
      if (frontEvents.length >= 1) {
        frontEvents.forEach(function (event) {
          if (event.isNormalPriority() === normal && !event._erased) {
            var eventList =
              $dataMap.events[event._eventId].pages[event._pageIndex].list;
            for (var i = 0; i < eventList.length; i++) {
              if (
                eventList[i].code === 111 &&
                eventList[i].parameters[0] === 1
              ) {
                if (eventList[i].parameters[1] === pd_IT_useVariableId) {
                  ...
                  if (eventList[i].parameters[2] === 0) {
                    ...
                    variableValue = $gameVariables.value(
                      eventList[i].parameters[3]
                    );
                  }
  ...
  Game_Map.prototype.isEventRunning = function () {
    if (pd_IT_Game_Map_isEventRunning.call(this)) {
      ...
      if (
        pd_IT_useVariableId != 0 &&
        $gameVariables.value(pd_IT_useVariableId) != 0
      ) {
