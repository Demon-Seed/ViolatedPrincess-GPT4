...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    switch (getCommandName(command)) {
      ...
      case "P_TARGET_ALL":
        ...
        break;
      case "P_TARGET_RANGE":
        $gameScreen.setPictureTargetRange(
          getArgNumber(args[0], 1),
          getArgNumber(args[1], 1)
        );
        break;
      case "P_TARGET_MULTI":
      ...
      case "P_SPIN_RELATIVE":
        ...
        $gameScreen.spinPictureRelative(
          getArgNumber(args[0], 1),
          getArgNumber(args[1]),
          spinRelativeDuration
        );
  ...
  Game_Interpreter.prototype.transPictureNumber = function (handler) {
    ...
    if ($gameSystem.pictureNumVariable) {
      ...
      this._params[0] = $gameVariables
        .value(this._params[0])
        .clamp(1, $gameScreen.maxPictures());
      result = handler();
  ...
  Game_Screen.prototype.spinPictureRelative = function (
    pictureId,
    relativeRotation,
    duration
  ) {
  ...
  Game_Screen.prototype.shakePicture = function (
    pictureId,
    power,
    speed,
    rotation,
  ...
  Game_Screen.prototype.setOutOfScreenShakePicture = function (
    pictureId,
    value
  ) {
    if (this._pictureTargetAll > 0) {
  ...
  Game_Screen.prototype.isTargetPicture = function (number) {
    switch (this._pictureTargetAll) {
      case 2:
        return (
          this._pictureTargetStart <= number && this._pictureTargetEnd >= number
        );
      case 3:
  ...
  Game_Picture.prototype.x = function () {
    return (
      _Game_Picture_x.apply(this, arguments) +
      this.getShakeX() -
  ...
  Game_Picture.prototype.spinRelative = function (
