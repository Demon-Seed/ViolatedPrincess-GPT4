...
Game_MapGenerator.prototype.initialize = function () {
  this._wallHeight = Number(
    PluginManager.parameters("SAN_MapGenerator")["WallHight"]
  );
  this._showOuterWall =
    PluginManager.parameters("SAN_MapGenerator")["ShowOuterWall"] === "ON";
...
Game_MapGenerator.prototype.isGround = function (x, y) {
  ...
  return (
    ["room", "pass", "start", "goal"].indexOf(this._symbolMap[x][y]) !== -1
  );
};
...
// イベントの設置
Game_MapGenerator.prototype.setEvent = function (
  event,
  targetSymbols,
  targetArea
) {
  ...
  for (var x = targetArea.x; x < targetArea.x + targetArea.w && !canSet; x++) {
    for (
      var y = targetArea.y;
      y < targetArea.y + targetArea.h && !canSet;
      y++
    ) {
...
Game_MapGenerator.prototype.setStart = function () {
  ...
  var event = new Game_Event(
    $gameMap.mapId(),
    this.dataMapEventsXy(refXY.x, refXY.y)[0].id
  );
...
Game_MapGenerator.prototype.autoTileId = function (x, y, z) {
  ...
  if (!Tilemap.isWallSideTile(baseTileId)) {
    ...
    [1, 2, 3, 4, 6, 7, 8, 9].forEach(function (direction) {
      ...
      var roundTileId =
        this._symbolTable[this._symbolMap[dx][dy]].baseTileId[z];
      if (Tilemap.isSameKindTile(baseTileId, roundTileId)) {
        candidateTileIds = candidateTileIds.filter(function (Id) {
          return (
            Game_MapGenerator.tileIdsFloor.connect[direction].indexOf(Id) !== -1
          );
        }); // 同種オートタイルの場合候補タイルIDから接続タイルIDを選択
      } else {
        candidateTileIds = candidateTileIds.filter(function (Id) {
          return (
            Game_MapGenerator.tileIdsFloor.noConnect[direction].indexOf(Id) !==
            -1
          );
    ...
    for (
      var by = y;
      this._symbolMap[x][y] === this._symbolMap[x][by + 1];
      by++
    ); // 壁の下端
    for (
      var ty = y;
      this._symbolMap[x][y] === this._symbolMap[x][ty - 1];
      ty--
    ); // 壁の上端
    // 上下の処理
    [2, 8].forEach(function (direction) {
      ...
      if (Tilemap.isSameKindTile(baseTileId, roundTileId)) {
        candidateTileIds = candidateTileIds.filter(function (Id) {
          return (
            Game_MapGenerator.tileIdsWall.connect[direction].indexOf(Id) !== -1
          );
        }); // 同種オートタイルの場合候補タイルIDから接続タイルIDを選択
      } else {
        candidateTileIds = candidateTileIds.filter(function (Id) {
          return (
            Game_MapGenerator.tileIdsWall.noConnect[direction].indexOf(Id) !==
            -1
          );
        }); // 異種オートタイルの場合候補タイルIDから非接続タイルIDを選択
      }
    ...
    [4, 6].forEach(function (direction) {
      ...
      var upperSideTileId =
        this._symbolTable[this._symbolMap[dx][ty]].baseTileId[z];
      var downerSideTileId =
        this._symbolTable[this._symbolMap[dx][by]].baseTileId[z];
      if (
        (Tilemap.isWallTile(upperSideTileId) ||
          Tilemap.isRoofTile(upperSideTileId)) &&
        (Tilemap.isWallTile(downerSideTileId) ||
          Tilemap.isRoofTile(downerSideTileId))
      ) {
        candidateTileIds = candidateTileIds.filter(function (Id) {
          return (
            Game_MapGenerator.tileIdsWall.connect[direction].indexOf(Id) !== -1
  ...
  return (
    this._symbolTable[this._symbolMap[x][y]].baseTileId[z] + candidateTileIds[0]
  );
};
...
Game_MapGenerator.prototype.notGroundToPass = function (x, y) {
  ...
  if (x < 2 || $gameMap.width() - 2 <= x) {
    return;
  }
  if (y < wH + 2 || $gameMap.height() - wH - 2 <= y) {
    return;
  }
  if (!this.isGround(x, y)) {
...
Game_MapGenerator.prototype.anyToSpace = function (x, y) {
  if (x < 0 || $gameMap.width() <= x) {
    return;
  }
  if (y < 0 || $gameMap.height() <= y) {
    return;
  }
  this._symbolMap[x][y] = "space";
};
...
Game_MapGeneratorRoomAndPass.prototype = Object.create(
  Game_MapGenerator.prototype
);
Game_MapGeneratorRoomAndPass.prototype.constructor =
  Game_MapGeneratorRoomAndPass;
...
Game_MapGeneratorRoomAndPass.prototype.generateMap = function () {
  this._minRoomSize = Number(
    PluginManager.parameters("SAN_MapGenerator")["MinRoomSize"]
  );
  this._maxRoomSize = Number(
    PluginManager.parameters("SAN_MapGenerator")["MaxRoomSize"]
  );
...
// 隣り合うブロックのリスト作成
Game_MapGeneratorRoomAndPass.prototype.makeAdjacentBlockIndexList =
  function () {
    for (var crntIndex = 0; crntIndex < this._blocks.length; crntIndex++) {
      ...
      for (var tgetIndex = 0; tgetIndex < this._blocks.length; tgetIndex++) {
        ...
        var matchH =
          (tgetBlock.x <= crntBlock.x + crntBlock.w &&
            tgetBlock.x >= crntBlock.x) ||
          (tgetBlock.x + tgetBlock.w <= crntBlock.x + crntBlock.w &&
            tgetBlock.x + tgetBlock.w >= crntBlock.x) ||
          (crntBlock.x <= tgetBlock.x + tgetBlock.w &&
            crntBlock.x >= tgetBlock.x) ||
          (crntBlock.x + crntBlock.w <= tgetBlock.x + tgetBlock.w &&
            crntBlock.x + crntBlock.w >= tgetBlock.x);
        var matchV =
          (tgetBlock.y <= crntBlock.y + crntBlock.h &&
            tgetBlock.y >= crntBlock.y) ||
          (tgetBlock.y + tgetBlock.h <= crntBlock.y + crntBlock.h &&
            tgetBlock.y + tgetBlock.h >= crntBlock.y) ||
          (crntBlock.y <= tgetBlock.y + tgetBlock.h &&
            crntBlock.y >= tgetBlock.y) ||
          (crntBlock.y + crntBlock.h <= tgetBlock.y + tgetBlock.h &&
            crntBlock.y + crntBlock.h >= tgetBlock.y);
        if (adjacentT && matchH) {
...
Game_MapGeneratorRoomAndPass.prototype.makeRooms = function () {
  this._blocks.forEach(function (block) {
    var roomW =
      this._minRoomSize +
      Math.randomInt(
        block.w - (this._wallHeight + 1) * 2 - this._minRoomSize - 2
      );
    var roomH =
      this._minRoomSize +
      Math.randomInt(
        block.h - (this._wallHeight + 1) * 2 - this._minRoomSize - 2
      );
    ...
    var roomX =
      ...
      (this._wallHeight + 1) +
      1 +
      Math.randomInt(block.w - roomW - (this._wallHeight + 1) * 2 - 1);
    var roomY =
      block.y +
      (this._wallHeight + 1) +
      1 +
      Math.randomInt(block.h - roomH - (this._wallHeight + 1) * 2 - 1);
    var room = {
      x: roomX,
      y: roomY,
      w: roomW,
...
Game_MapGeneratorRoomAndPass.prototype.makePasses = function () {
  ...
  for (
    var crntIndex = 0;
    crntIndex < this._adjacentBlockIndexList.length;
    crntIndex++
  ) {
    cache[crntIndex] = [];
    ...
    for (var direction in this._adjacentBlockIndexList[crntIndex]) {
      ...
      tgetIndexList.forEach(function (tgetIndex) {
        if (
          cache[tgetIndex] !== undefined &&
          cache[tgetIndex].indexOf(crntIndex) !== -1
        ) {
          return;
        }
...
Game_MapGeneratorRoomAndPass.prototype.isSplitableByRoomNum = function () {
  ...
  if (
    this._blocks.length >= this._minRooms &&
    this.randBool(
      (this._blocks.length - this._minRooms + 1) /
...
Game_Character.prototype.isSameRoomWithPlayer = function () {
  if ($gameMap.mapGenerator().constructor === Game_MapGenerator) {
    return true;
  }
...
// イベントクラスの初期化
Sanshiro.Game_MapGenerator.Game_Event_initialize =
...
// プラグインコマンド
Sanshiro.Game_MapGenerator.Game_Interpreter_pluginCommand =
  Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  Sanshiro.Game_MapGenerator.Game_Interpreter_pluginCommand.call(
    this,
    command,
    args
  );
