...
(function () {
  ...
  var convertArrayParam = function (param) {
    ...
    if (param !== undefined) {
      try {
        ...
        console.group();
        console.error(
          "%cParameter is invalid ! SNH_MsgNameLableのパラメータ準備に失敗しました。",
  ...
  var nameAutoMode = Boolean(
    parameters["文字サイズ自動調整"] === "true" || false
  );
  ...
  (function initPartsList() {
    if (param.NamePlate && param.NamePlate.length > 0) {
      ...
      plate.gradationVertical = Boolean(
        plate.gradationVertical === "true" || false
      );
    ...
    if (param.AttachmentRect && param.AttachmentRect.length > 0) {
      for (let rect of param.AttachmentRect) {
        ...
        rect.gradationVertical = Boolean(
          rect.gradationVertical === "true" || false
        );
    ...
    if (param.NamePlatePictures && param.NamePlatePictures.length > 0) {
      ...
      pPicture.autoScaleMode = Boolean(
        pPicture.autoScaleMode === "true" || false
      );
  ...
  Window_Message.prototype.snhDrawNameLabel = function () {
    ...
    nameBitmap.outlineColor = scene._messageWindow.textColor(
      nameLabelFontOutLineColor
    );
    ...
    if (width > 0) {
      ...
      if (wNamePlate.gradationMode) {
        namePlateBitmap.gradientFillRect(
          0,
          0,
          width,
          wNamePlate.height,
          "rgba(" +
            wNamePlate.color1.red +
            "," +
            wNamePlate.color1.green +
            "," +
            wNamePlate.color1.blue +
            "," +
            wNamePlate.color1.opacity +
            ")",
          "rgba(" +
            wNamePlate.color2.red +
            "," +
            wNamePlate.color2.green +
            "," +
            wNamePlate.color2.blue +
            "," +
            wNamePlate.color2.opacity +
            ")",
          wNamePlate.gradationVertical
        );
      } else {
        namePlateBitmap.fillRect(
          0,
          0,
          width,
          wNamePlate.height,
    ...
    if (namePlatePicture && namePlatePicture.length > 0) {
      ...
      platePictureSprite.move(
        namePlatePicture[0].positionX,
        namePlatePicture[0].positionY
      );
    ...
    if (nameLabelFontSize < wNamePlate.height - wNamePlate.paddingY * 2) {
      ...
      if (nameLabelAlignY === "up") {
        ...
        yChosei =
          wNamePlate.paddingY +
          (wHeight - wNamePlate.paddingY - nameLabelFontSize) / 2;
      } else if (nameLabelAlignY === "down") {
        yChosei =
          wNamePlate.paddingY +
          (wHeight - nameLabelFontSize - wNamePlate.paddingY);
      }
    } else {
      yChosei = wNamePlate.paddingY;
    }
