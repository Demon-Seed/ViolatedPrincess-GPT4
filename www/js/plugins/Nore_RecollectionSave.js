...
(function (Nore) {
  ...
  var _Window_SavefileList_prototype_drawFileId =
  ...
  var RecoSaveManager = /** @class */ (function () {
    ...
    RecoSaveManager.prototype.save = function (saveFileName) {
      ...
      console.log(
        "回想セーブを実行 " + slotId + " " + saveId + " " + saveFileName
      );
  ...
  Scene_File.prototype.onUp = function () {
    ...
    this._listWindow.select(
      (before % maxCols) + this._listWindow.maxItems() - maxCols
    );
  };
  ...
  Scene_Load.prototype.onSavefileOk = function () {
    ...
    if (
      saveFileId < getRecoStartId() ||
      saveFileId == Nore.AUTO_SAVE_DUMMY_ID
    ) {
  ...
  var Window_SavefileList2 = /** @class */ (function (_super) {
    ...
    Window_SavefileList2.prototype.drawThumbnail = function (
      info,
      thumbRect,
      valid
    ) {
      ...
      if (savefileId >= 0 && info.thumbnail) {
        ...
        if (!thunmbBitmap_1.isReady()) {
          ...
          var empty = ImageManager.loadBusyThumbBitmap(
            thumbRect.width,
            thumbRect.height
          );
        ...
        thunmbBitmap_1.addLoadListener(function () {
          ...
          if (
            _this.topIndex() <= listIndex &&
            _this.bottomIndex() >= listIndex
          ) {
            sprite_1.visible = true;
            sprite_1.bitmap = thunmbBitmap_1;
            sprite_1.bitmap.paintOpacity = valid
              ? 255
              : _this.translucentOpacity();
          } else {
  ...
  var Window_SavePage = /** @class */ (function (_super) {
    ...
    Window_SavePage.prototype.itemRect = function (index) {
      ...
      rect.x =
        (index % maxCols) * (rect.width + this.spacing()) - this._scrollX + 40;
    ...
    Window_SavePage.prototype.drawItem = function (index) {
      ...
      if (data >= RECOLLECTION_PAGE - 1) {
        ...
        this.drawText(
          text.format(data + 2 - RECOLLECTION_PAGE),
          rect.x,
          -5,
          40,
          30,
