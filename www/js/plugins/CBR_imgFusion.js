...
CBR.imgFusion.onLoad = function (e) {
  ...
  if (!CBR.imgFusion.check(this.CBR_imgFusion._name)) {
    //今後何があるかわらないから
    return;
  }
  this.clear();
...
CBR.imgFusion.check = function (name) {
  ...
  if (key !== null) {
    for (var i = 0, len = ary[key].add_ary.length; i < len; i++) {
      //素材読みこむよー
      var bitmap = ImageManager.loadBitmap(
        "img/pictures/",
        ary[key].add_ary[i].slice(0, -4),
        undefined,
        true
      ...
      if (bitmap._loadingState != "loaded") {
        ...
        var test = function () {
          ...
          var bitmap = ImageManager.loadBitmap(
            "img/pictures/",
            this._CBR_imgFusion_parent.slice(0, -4),
            undefined,
            true
          );
        ...
        bitmap._image.addEventListener(
          "load",
          (bitmap._loadListener = test.bind(bitmap))
        );
        //ダメだったよ
        temp = false;
        break;
...
ImageManager.loadPicture = function (filename, hue) {
  ...
  var temp = false;
  for (
    var i = 0, len = $gameSystem._CBR_imgFusion._fusionList.length;
    i < len;
    i++
  ) {
  ...
  if (temp) {
    ...
    if (!bitmap.CBR_imgFusion) {
      ...
      if (
        ...
        bitmap._loadingState == "decrypting"
      ) {
