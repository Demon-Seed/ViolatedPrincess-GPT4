...
(function (Nore) {
  ...
  Nore.RIGHT_TREE_OFFSET = Parameters["rightTreeOffset"]
    ? Math.trunc(Parameters["rightTreeOffset"])
    : 15;
  Nore.CLOSE_RELATIVE_X = Parameters["closeRelativeX"]
    ? Math.trunc(Parameters["closeRelativeX"])
    : 0;
  Nore.CLOSE_RELATIVE_Y = Parameters["closeRelativeY"]
    ? Math.trunc(Parameters["closeRelativeY"])
    : 0;
  Nore.CLOSE_RELATIVE_NAME_Y = Parameters["closeRelativeNameY"]
    ? Math.trunc(Parameters["closeRelativeNameY"])
    : 0;
  // 設定
  var FONT_NAME = "游ゴシック";
  var FONT_SIZE = 18;
  ...
  Scene_Boot.loadSystemImages = function () {
    ...
    loader.add("HeaderFont", "./fonts/bgfont.fnt").load(function () {});
  };
  ...
  Scene_Map.prototype.showFamilyTree = function () {
    if (!this._familyTreeWindow) {
      ...
      this._familyTreeWindow = new Nore.Window_FamilyTree(
        0,
        0,
        Graphics.boxWidth,
        Graphics.boxHeight
...
var Game_FamilyTree = /** @class */ (function () {
  ...
  Game_FamilyTree.prototype.addGrandchild = function (childCount, id) {
    if (id) {
      ...
      if (!parent_1) {
        console.error(
          "孫を追加しようとしましたが、" + id + "のIDの子供が存在しません"
        );
        return;
      }
      var newChild = new Game_Child(
        parent_1.taneoyaId(),
        parent_1.generation() + 1
      );
      newChild.plusBrother(childCount - 1);
      ...
      if (!this._lastChild) {
        console.error(
          "孫を追加しようとしましたが、直前に産まれた子供が存在しません"
        );
        return;
      }
      var newChild = new Game_Child(
        this._lastChild.taneoyaId(),
        this._lastChild.generation() + 1
      );
      newChild.plusBrother(childCount - 1);
...
(function (Nore) {
  var Scene_FamilyTree = /** @class */ (function (_super) {
    ...
    Scene_FamilyTree.prototype.createFamilyTreeWindow = function () {
      this._familyTreeWindow = new Window_FamilyTree(
        0,
        0,
        Graphics.boxWidth,
        Graphics.boxHeight
  ...
  var Sprite_Child = /** @class */ (function (_super) {
    ...
    Sprite_Child.prototype.createNameSprite = function (
      taneoyaId,
      x,
      y,
      isChild,
  ...
  var Sprite_ChildLeft = /** @class */ (function (_super) {
    ...
    Sprite_ChildLeft.prototype.addGrandChild = function (y, child) {
      ...
      if (child.brotherCount() > 1) {
        ...
        this.drawLineHorizontal(
          409 - INTERBAL_X * maxChild,
          y + hh - 1,
          INTERBAL_X * maxChild + 5
        );
        var xx_1 = 0;
        var yy = y + 12;
        for (var i = 0; i < maxChild; i++) {
          this.drawLineVertial(
            289 - INTERBAL_X * i,
            y + MARGIN_BETWEEN_FATHER_AND_CHILDREN - 9,
            50
          );
    ...
    Sprite_ChildLeft.prototype.drawBgLines = function (child, y) {
      ...
      this.drawLineHorizontal(
        129 + xx,
        60 + y,
        INTERBAL_X * Math.min(bro1, MAX_CHILDREN - 1) + 1
      );
  ...
  var Sprite_ChildRight = /** @class */ (function (_super) {
    ...
    Sprite_ChildRight.prototype.drawBgLines = function (child, y) {
      ...
      this.putDot(
        21 + INTERBAL_X * Math.min(bro1, MAX_CHILDREN - 1),
        60 + y + 1,
        LINE_COLOR
      );
  ...
  var Sprite_VerticalLine = /** @class */ (function (_super) {
    ...
    function Sprite_VerticalLine(centerLineInfo) {
      ...
      for (var i = 0; i < yList.length; i++) {
        ...
        _this.drawLineHorizontal(
          0,
          y - centerLineInfo.initialY() + 4,
          offsetX + 1
        );
      }
      ...
      if (centerLineInfo.maxLeft() > centerLineInfo.maxRight()) {
        _this.bitmap.fillRect(
          offsetX + 1,
          centerLineInfo.maxLeft() + 2,
          3,
          1,
          LINE_COLOR
        );
