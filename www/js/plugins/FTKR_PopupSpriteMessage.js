...
(function () {
  ...
  var _PSM_Game_Interpreter_pluginCommand =
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    switch (command) {
      ...
      case "ポップアップ消去":
      case "ERASE_POPUP":
        $gameParty.requestErasePopupMessage(
          setArgNum(args[0]),
          setArgNum(args[1])
        );
  ...
  var _PSM_Game_Interpreter_updateWaitMode =
  ...
  Game_Party.prototype.setPopupMessage = function (
    messageId,
    x1,
    y1,
    duration,
  ...
  Game_Party.prototype.requestErasePopupMessage = function (
    messageId,
    duration
  ) {
    if (!this.psmMessage(messageId)) return;
  ...
  Game_Party.prototype.eraseDuration = function (messageId) {
    return (
      (this.psmMessage(messageId) &&
        this.psmMessage(messageId).eraseDuration) ||
      0
    );
  };
  ...
  Game_Party.prototype.movePopupMessage = function (
    messageId,
    x2,
    y2,
    duration
  ...
  Game_Party.prototype.rotatePopupMessage = function (
    messageId,
    speed,
    rotate
  ) {
  ...
  Game_Party.prototype.changeColorMessage = function (
    messageId,
    color,
    opacity,
    duration
  ...
  //  半角スペース用の制御文字を追加
  //=============================================================================
  var _PSM_Window_Base_convertEscapeCharacters =
  ...
  // メッセージスプライトを作成
  //=============================================================================
  var _PSM_Spriteset_Base_createUpperLayer =
  ...
  Sprite_FtPopupMessage.prototype.setupDynamicText = function (text) {
    ...
    var width = text
      ? (this._italic ? size * 1.5 : size) * text.length
      : size + 8;
  ...
  Sprite_FtPopupMessage.prototype.setupFlashEffect = function (
    flashColor,
    duration
  ) {
  ...
  Sprite_FtPopupMessage.prototype.updatePosition = function () {
    ...
    if (this._moveDuration > 0) {
      ...
      this.x = this.textX(
        Math.floor(
          message.x +
            (message.dx - message.x) *
              (1 - this._moveDuration / message.moveDuration)
        )
      );
      this.y = this.textY(
        Math.floor(
          message.y +
            (message.dy - message.y) *
              (1 - this._moveDuration / message.moveDuration)
        )
      );
  ...
  Sprite_FtPopupMessage.prototype.updateDuration = function () {
    if (
      this._duration == -1 &&
      $gameParty.isErasePopupMessage(this._messageId)
    ) {
  ...
  Sprite_FtPopupMessage.prototype.setNextColor = function (index, message) {
    return Math.floor(
      this._colorA[index] +
        (message.dcolor[index] - this._colorA[index]) *
          (1 - this._colorDuration / message.colorDuration)
    );
  };
  ...
  Sprite_FtPopupMessage.prototype.updateOpacity = function () {
    ...
    if (this._colorDuration > 0) {
      ...
      this.updateColor();
      if (message.dopacity >= 0)
        this.opacity = Math.floor(
          message.opacity +
            (message.dopacity - message.opacity) *
              (1 - this._colorDuration / message.colorDuration)
        );
