...
(() => {
  ...
  const param_NameWindow = JSON.parse(
    parameters["Name Window"] || "{}",
    paramReplace
  );
  const param_FaceWindow = JSON.parse(
    parameters["Face Window"] || "{}",
    paramReplace
  );
  ...
  const param_DefaultRubySize = Number(
    baseParameters["Default Ruby Size"] || 14
  );
  ...
  const _Window_Message_clearFlagsMessageEx =
    Window_Message.prototype.clearFlagsMessageEx;
  ...
  const _Window_Message_terminateMessageEx =
    Window_Message.prototype.terminateMessageEx;
  ...
  const _Window_Message_updateSpeakerName =
    Window_Message.prototype.updateSpeakerName;
  ...
  Window_FaceBox.prototype.windowX = function () {
    ...
    const targetX =
      messageWindow.x + (this._right ? messageWindow.width - this.width : 0);
    if (this._onBox) {
  ...
  Window_FaceBox.prototype.updateMirror = function () {
    const contentsSprite = this._windowContentsSprite || this._contentsSprite;
  ...
  if (Utils.RPGMAKER_NAME === "MZ") {
    ...
    Window_NameBox.prototype.updatePadding = function () {
      this.padding = param_NameWindow["Padding"] || $gameSystem.windowPadding();
    };
    ...
    Window_NameBox.prototype.resetFontSettings = function () {
      ...
      this.contents.fontSize =
        param_NameWindow["Font Size"] || _fontSize.call(this);
      this.resetTextColor();
    };

    const _Window_NameBox_resetTextColor = __base(
      Window_NameBox.prototype,
      "resetTextColor"
    );
    ...
    Window_NameBox.prototype.createTextState = function (text, x, y, width) {
      const textState = Window_Base.prototype.createTextState.apply(
        this,
        arguments
      );
    ...
    const _Window_NameBox_processEscapeCharacter = __base(
      Window_NameBox.prototype,
      "processEscapeCharacter"
    );
    Window_NameBox.prototype.processEscapeCharacter = function (
      code,
      textState
    ) {
      if (code === "RB") {
    ...
    Window_NameBox.prototype.processRubyCharacter = function (
      textState,
      texts
    ) {
      const c = texts[0];
      ...
      if (textState.drawing) {
        if (param_NameWindow["Ruby Color"]) {
          this.changeTextColor(
            "rgb(%1)".format(param_NameWindow["Ruby Color"])
          );
        }
  ...
  Window_NameBoxMV.prototype.processEscapeCharacter = function (
    code,
    textState
  ) {
    if (code === "RB") {
  ...
  Window_NameBoxMV.prototype.processRubyCharacter = function (
    textState,
    texts
  ) {
    const c = texts[0];
  ...
  Window_NameBoxMV.prototype.calcTextHeight = function (textState, all) {
    const textHeight = Window_Base.prototype.calcTextHeight.call(
      this,
      textState,
      all
    );
