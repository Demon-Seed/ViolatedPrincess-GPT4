...
(function () {
  ...
  //=============================================================================
  var param = {};
  param.returnToFirstCell = getParamBoolean([
    "ReturnToFirstCell",
    "最初のセルに戻る",
  ]);
  ...
  Game_Interpreter.prototype.pluginCommandPictureAnimation = function (
    command,
    args
  ) {
    var pictureNum,
    ...
    switch (getCommandName(command)) {
      ...
      case "ピクチャのアニメーション準備":
        ...
        $gameScreen.setPicturesAnimation(
          cellNumber,
          frameNumber,
          direction,
          fadeDuration
      ...
      case "ピクチャのアニメーション開始":
        ...
        if (picture)
          picture.startAnimationFrame(animationType, false, customArray);
        break;
      case "PA_START_LOOP":
      case "ピクチャのループアニメーション開始":
        ...
        picture = $gameScreen.picture(pictureNum);
        if (picture)
          picture.startAnimationFrame(animationType, true, customArray);
        break;
      case "PA_STOP":
      case "ピクチャのアニメーション終了":
        pictureNum = getArgNumber(args[0], 1, $gameScreen.maxPictures());
      ...
      case "ピクチャのアニメーションセル設定":
        ...
        picture = $gameScreen.picture(pictureNum);
        if (picture) {
          if (wait === "ウェイトあり" || wait.toUpperCase() === "WAIT")
  ...
  //=============================================================================
  Game_Screen.prototype.setPicturesAnimation = function (
    cellNumber,
    frameNumber,
    direction,
    fadeDuration
  ...
  Object.defineProperty(Game_Picture.prototype, "cell", {
    get: function () {
      ...
      switch (this._animationType) {
        case 3:
          return (this._customArray[this._cellCount] - 1).clamp(
            0,
            this._cellNumber - 1
          );
        case 2:
          return (
            this._cellNumber -
            1 -
            Math.abs(this._cellCount - (this._cellNumber - 1))
          );
        case 1:
  ...
  Game_Picture.prototype.linkToVariable = function (variableNumber) {
    this._linkedVariable = variableNumber.clamp(
      1,
      $dataSystem.variables.length
    );
  };
  ...
  Game_Picture.prototype.isEndFirstLoop = function () {
    return (
      this._cellCount ===
      (param.returnToFirstCell ? 0 : this.getCellNumber() - 1)
    );
  };
  ...
  Game_Picture.prototype.setAnimationFrameInit = function (
    cellNumber,
    frameNumber,
    direction,
    fadeDuration,
  ...
  Game_Picture.prototype.startAnimationFrame = function (
    animationType,
    loopFlg,
    customArray
  ) {
  ...
  Sprite_Picture.prototype.loadAnimationBitmap = function () {
    ...
    for (var i = 1; i < cellNumber; i++) {
      var filename =
        this._pictureName.substr(0, this._pictureName.length - cellDigit) +
        i.padZero(cellDigit);
