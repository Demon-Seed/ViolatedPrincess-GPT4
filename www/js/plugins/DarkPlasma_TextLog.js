...
(function () {
  ...
  const disableLoggingSwitch = Number(
    pluginParameters["Disable Logging Switch"]
  );
  const openLogKey = String(pluginParameters["Open Log Key"]);
  const disableShowLogSwitch = Number(
    pluginParameters["Disable Show Log Switch"]
  );
  const showLogWindowWithoutText =
    String(pluginParameters["Show Log Window Without Text"]) !== "false";
  ...
  const settings = {
    ...
    eventLogSplitter: String(
      pluginParameters["Event Log Splitter"] ||
        "-------------------------------"
    ),
    autoEventLogSplit:
      String(pluginParameters["Auto Event Split"] || "true") === "true",
    includeName:
      String(pluginParameters["Include Name In Log"] || "true") === "true",
    includeScrollText:
      String(pluginParameters["Include Scroll Text In Log"]) === "true",
    includeChoice: String(pluginParameters["Include Choice In Log"]) === "true",
    choiceFormat: String(pluginParameters["Choice Format"]),
    choiceColor: String(pluginParameters["Choice Color"]).startsWith("#")
      ? String(pluginParameters["Choice Color"])
      : Number(pluginParameters["Choice Color"]),
    includeChoiceCancel:
      String(pluginParameters["Include Choice Cancel In Log"] || "true") ===
      "true",
    choiceCancelText: String(
      pluginParameters["Choice Cancel Text In Log"] || "キャンセル"
    ),
    smoothBackFromLog:
      String(pluginParameters["Smooth Back From Log"] || "true") === "true",
  };
  ...
  class EventTextLog {
    ...
    addMessageLog(text) {
      ...
      if (
        settings.logMessageCount > 0 &&
        this._messages.length > settings.logMessageCount
      ) {
  ...
  class Window_TextLog extends Window_Base {
    ...
    initialize() {
      ...
      if (
        settings.logMessageCount > 0 &&
        this._viewTexts.length > settings.logMessageCount
      ) {
        this._viewTexts.splice(
          0,
          this._viewTexts.length - settings.logMessageCount
        );
      }
  ...
  function moveToPrevLog() {
    ...
    if (
      settings.logEventCount > 0 &&
      pastEventLog.length > settings.logEventCount
    ) {
  ...
  function isTextLogEnabled() {
    return (
      ...
      (disableShowLogSwitch === 0 || !$gameSwitches.value(disableShowLogSwitch))
    );
  }
  ...
  Game_Interpreter.prototype.isParallelEvent = function () {
    return (
      this._eventId !== 0 &&
      this.isOnCurrentMap() &&
      $gameMap.event(this._eventId).isTriggerIn([4])
    );
  };
