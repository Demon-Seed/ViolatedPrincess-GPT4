...
(function () {
  ...
  if (typeof PluginManager === "undefined") {
    ...
    Laurus.Text2Frame.IsDebug =
      String(Laurus.Text2Frame.Parameters["IsDebug"]) == "true" ? true : false;
  ...
  Game_Interpreter.prototype.pluginCommandText2Frame = function (
    command,
    args
  ) {
      ...
      case "メッセージをイベントにインポート":
        ...
        if (
          ...
          (args[4].toLowerCase() === "true" ||
            args[4].toLowerCase() === "false")
        ) {
          Laurus.Text2Frame.IsOverwrite =
            args[4].toLowerCase() === "true" ? true : false;
        ...
        if (args[0] || args[1]) {
          Laurus.Text2Frame.TextPath = `${BASE_PATH}${PATH_SEP}${Laurus.Text2Frame.FileFolder}${PATH_SEP}${Laurus.Text2Frame.FileName}`;
          Laurus.Text2Frame.MapPath = `${BASE_PATH}${path.sep}data${
            path.sep
          }Map${("000" + Laurus.Text2Frame.MapID).slice(-3)}.json`;
        }
        break;
    ...
    const getBlockStatement = function (scenario_text, statement) {
      ...
      while (block !== null) {
        ...
        let match_text_list = match_text
          .replace(/^\n/, "")
          .replace(/\n$/, "")
          .split("\n");
        let event_list = [];
    ...
    for (let i = 0; i < text_lines.length; i++) {
      ...
      if (text) {
        ...
        let replay_bgm =
          text.match(/<replaybgm>/i) || text.match(/<BGMの再開>/);
        ...
        let rotate_picture =
          ...
          text.match(
            /<ピクチャの回転\s*:\s*(\d{1,2})\s*,\s*(-?\d{1,2})\s*>/i
          ) ||
        ...
        let conditional_branch_else =
          text.match(/\s*<else>/i) || text.match(/\s*<それ以外のとき>/);
        let conditional_branch_end =
          text.match(/\s*<end>/i) || text.match(/\s*<分岐修了>/);
          ...
          if (play_bgm[1]) {
            ...
            if (name.toUpperCase() === "NONE" || name === "なし") {
              ...
              event_command_list.push(
                getPlayBgmEvent(name, volume, pitch, pan)
              );
            }
          }
          continue;
        ...
        if (change_battle_bgm) {
          if (change_battle_bgm[1]) {
            ...
            if (name.toUpperCase() === "NONE" || name === "なし") {
              event_command_list.push(
                getChangeBattleBgmEvent("", volume, pitch, pan)
              );
            } else {
              event_command_list.push(
                getChangeBattleBgmEvent(name, volume, pitch, pan)
              );
            }
          }
          continue;
        ...
        if (play_bgs) {
          if (play_bgs[1]) {
            ...
            if (name.toUpperCase() === "NONE" || name === "なし") {
              ...
              event_command_list.push(
                getPlayBgsEvent(name, volume, pitch, pan)
              );
            }
          }
          continue;
        ...
        const set_reg_list = set_operation_list.map(
          (x) =>
            `<${x} *: *(\\d+\\-?\\d*) *, *(${control_variable_arg_regex}) *>`
        );
        ...
        const switch_tag = text.match(
          new RegExp(switch_reg_list.join("|"), "i")
        );

        const self_switch_operation_list = [
          "ssw",
          "selfswitch",
          "セルフスイッチ",
        ];
        ...
        const getControlTag = function (operator, operand1, operand2) {
          ...
          if (operator == "switch") {
            const switch_tag = operand2.match(
              /on|オン|1|true|off|オフ|0|false/i
            );
            if (switch_tag) {
              return getControlSwitch(
                start_pointer,
                end_pointer,
                switch_tag[0]
              );
        ...
        if (self_switch_tag) {
          ...
          event_command_list.push(
            getControlTag("selfswitch", operand1, operand2)
          );
          continue;
        }
        ...
        const timer_stop = text.match(
          new RegExp(timer_stop_reg_list.join("|"), "i")
        );
